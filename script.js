/* =========================================
   ...
   Features: Neon Flow, FEA Mesh, Mech-UI, HUD, Gears
   ========================================= */
...
.card:hover::after { background: conic-gradient(transparent, transparent, var(--accent-green), #ffffff); animation-duration: 2s; }
...
/* --- 5. SYNCHRO-GEAR & HUD & MODAL --- */
.gear-container {
    position: fixed; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; overflow: hidden;
}
.mech-gear { position: absolute; transition: transform 0.1s linear; }
.mech-gear path { fill: rgba(255, 255, 255, 0.1) !important; filter: drop-shadow(0 0 5px rgba(255, 255, 255, 0.2)); }
.gear-1 { width: 600px; height: 600px; top: -100px; right: -150px; transform-origin: center; }
.gear-2 { width: 400px; height: 400px; bottom: -50px; left: -100px; transform-origin: center; }

/* HUD Styles */
.hud-container {
    position: fixed; top: 20px; left: 20px; right: 20px; bottom: 20px;
    pointer-events: none; z-index: 100; display: flex; justify-content: space-between; flex-direction: column;
}
.hud-corner { position: absolute; width: 30px; height: 30px; opacity: 0.5; }
.c-tl { top: 0; left: 0; } .c-tr { top: 0; right: 0; } .c-bl { bottom: 0; left: 0; } .c-br { bottom: 0; right: 0; }
.hud-item {
    font-family: 'Courier New', monospace; font-size: 0.75rem; color: var(--accent-green);
    position: absolute; display: flex; align-items: center; gap: 8px; text-shadow: 0 0 5px rgba(74, 222, 128, 0.5);
}
.top-left { top: 5px; left: 40px; } .top-right { top: 5px; right: 40px; }
.bottom-left { bottom: 5px; left: 40px; } .bottom-right { bottom: 5px; right: 40px; }
.pulse-dot { width: 6px; height: 6px; background: var(--accent-green); border-radius: 50%; animation: blink 1s infinite; }
.scroll-bar-hud { width: 100px; height: 4px; background: rgba(255, 255, 255, 0.1); margin-left: 5px; }
#scroll-progress { height: 100%; width: 0%; background: var(--accent-green); }
...
@media (max-width: 768px) {
    ...
    body { cursor: auto; }
    h1 { font-size: 2.5rem; }
    .hud-container { display: none; } /* Hide HUD on mobile */
    .gear-container { opacity: 0.3; } /* Gears visible but faint */
    .gear-1 { width: 300px; height: 300px; right: -80px; }
    .gear-2 { width: 200px; height: 200px; left: -50px; }
}
